defmodule ApiServerWeb.ContractControllerTest do
  use ApiServerWeb.ConnCase

  alias ApiServer.ContractManagement

  @create_attrs %{cname: "some cname", cno: "some cno"}
  @update_attrs %{cname: "some updated cname", cno: "some updated cno"}
  @invalid_attrs %{cname: nil, cno: nil}

  def fixture(:contract) do
    {:ok, contract} = ContractManagement.create_contract(@create_attrs)
    contract
  end
  ...
  describe "update contract" do
    ...
    test "redirects when data is valid", %{conn: conn, contract: contract} do
      ...
      assert html_response(conn, 200) =~ "some updated cname"
    end
